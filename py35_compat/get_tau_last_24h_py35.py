...
def fetch_goes_data(
    ...
    if missing_products:
        ...
        raise RuntimeError("GOES-{} returned no {} data".format(satellite, missing))

    print("GOES-{} fetch complete: {} files processed".format(satellite, len(matches)))


def prune_cache(directory, cutoff_time):
    ...
    for file_path in directory.glob("*.nc"):
        ...
        if file_times is None:
            print("Keeping unrecognized cache file: {}".format(file_path.name))
            continue
    ...
    print("Cache cleanup for {}: removed {} old files".format(directory.name, removed_count))
    return removed_count


def convert_pwv_to_tau(pwv_values, a=0.04, b=0.017):
    """Convert precipitable water vapor in millimeters to tau at 220 GHz."""
    return [(a * float(value)) + b for value in pwv_values]


def calculate_tau(directory, start_time, end_time):
    ...
    if not finite_pairs:
        raise ValueError("No finite tau values were calculated from {}".format(directory))
...
def update_current_plot_symlink(plot_path):
    ...
    if current_plot_path.exists() and not current_plot_path.is_symlink():
        raise FileExistsError(
            "Refusing to replace non-symlink file: {}".format(current_plot_path)
        )

    temporary_link = plot_path.parent / ".current_plot.{}.tmp".format(os.getpid())
    try:
...
def main():
    ...
    print("Query window: {} to {}".format(start_time.isoformat(), run_time.isoformat()))

    series_by_satellite = {}

    for satellite in SATELLITES:
        satellite_directory = CACHE_DIR / "goes_{}".format(satellite)
    ...
    plot_path = PLOTS_DIR / "tau_last_48h_{}.png".format(run_timestamp)
    ...
    print("Updated current plot link: {} -> {}".format(current_plot_path, plot_path.name))
